...
class PNEUMATrajectory:
    ...
    def plot_attribute(self, attribute, label, color, ax, alpha=1):
        """To plot an attribute"""
        ax.plot(self.df["frame"], self.df[attribute], c=color, label=label, alpha=alpha)
        return ax
    ...
    def smoother(self, attribute, window, algorithm="gaussian", plot=False, *kwargs):
        ...
        if algorithm == "gaussian":
            ...
            sm_attribute = (
                pd.Series(self.df[attribute])
                .rolling(min_periods=1, center=True, window=window)
                .mean()
            )
        elif algorithm == "sg":
            # same as moving average for polyorder 1
            sm_attribute = savgol_filter(
                list(self.df[attribute]),
    ...
    def detect_anomalies(self, n=2, b=15, model="xgb"):
        ...
        train_X, _, train_y, _ = train_test_split(X, y, test_size=0.2, random_state=123)
    ...
    def plot_process(self, image_path, anomaly_index):
        ...
        ax[0, 1].plot(
            self.df.frame, self.df.a_x, label="Original", linestyle="-", alpha=0.6
        )
        ax[0, 1].plot(
            self.df[np.abs(self.df.xgb - self.df.dsv) > tol].frame,
            self.df[np.abs(self.df.xgb - self.df.dsv) > tol].dsv,
            "rx",
            label="Anomalies",
        )
        ...
        if len(anomaly_index) != 0:
            ...
            ax[1, 0].plot(
                self.df.frame, self.df.g_s, label="GF with anomalies", color="r"
            )
            ax[1, 0].plot(
                self.df.frame, self.df.g_ns, label="GF without anomalies", color="k"
            )

            ax[1, 1].plot(self.df.frame, self.df.dv, label="Original", alpha=0.6)
            ax[1, 1].plot(
                self.df.frame, self.df.gdv, label="GF with anomalies", color="r"
            )
            ax[1, 1].plot(
                self.df.frame, self.df.g_na, label="GF without anomalies", color="k"
            )

            xmin, xmax = np.min(
                self.df[np.abs(self.df.xgb - self.df.dsv) > tol].frame - 50
...
if __name__ == "__main__":
    ...
    tol = 0.1  # to check the reconstruction loss in the acceleration subsequence
    merge_subsequences = (
        10  # to merge subsequences if they are within these many frames apart
    )
    ...
    for v_id in tqdm(list(all_ids)):
        ...
        vehicle.setter(
            "sv", vehicle.smoother("v", window=smoothing_window, algorithm="sg")
        )
        ...
        vehicle.setter(
            "northing",
            vehicle.df.apply(
                lambda x: np.round(utm.from_latlon(x["lat"], x["lon"])[1], 8), axis=1
            ),
        )
        vehicle.setter(
            "easting",
            vehicle.df.apply(
                lambda x: np.round(utm.from_latlon(x["lat"], x["lon"])[0], 8), axis=1
            ),
        )

        # calculate the speed in Km/h
        try:
            vehicle.df["pos_v"] = np.round(
                3.6
                * np.sqrt(
                    vehicle.get_derivative("northing") ** 2
                    + vehicle.get_derivative("easting") ** 2
                ),
                4,
            )
        ...
        try:
            ...
            vehicle.setter(
                "xgb", vehicle.detect_anomalies()
            )  # default params: b=4, n=3

            # check reconstruction loss with respect to the tolerance param
            vehicle.setter(
                "filtered_acc",
                vehicle.df.apply(
                    lambda x: x["dsv"] if np.abs(x.dsv - x.xgb) < tol else x.xgb, axis=1
                ),
            )
            ...
            vehicle.setter(
                "anomaly",
                vehicle.df.apply(
                    lambda x: 0 if np.abs(x.dsv - x.xgb) < tol else 1, axis=1
                ),
            )
            ...
            if len(ano_index) != 0:
                ...
                vehicle.setter(
                    "new_speed",
                    vehicle.df.apply(
                        lambda x: x["v"] if x.anomaly == 0 else x.filtered_speed, axis=1
                    ),
                )
                ...
                vehicle.setter(
                    "sg_ns",
                    vehicle.smoother("new_speed", smoothing_window, algorithm="sg"),
                )
                ...
                vehicle.setter(
                    "g_ns", vehicle.smoother("new_speed", smoothing_window_gaussian)
                )
