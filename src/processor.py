...
class PNEUMATrajectory:
	...
	def save_data(self, output_path, save_attributes):
		'''To read the csv file
		'''
		self.df[save_attributes].to_csv(output_path, index=False)
	...
	def detect_anomalies(self, 
		...
		Note that the regularization for the boosting model is depends on 
	...
	def plot_process(self, image_path):
		...
		ax[0,0].plot(self.df.frame, self.df.dsv, label="SF filter with anomalies", color='k')
		...
		# ax[1,0].plot(vehicle.df.frame, vehicle.df.sg_ns, label='SG-F without anomalies')
...
if __name__ == "__main__":
	...
	INPUT_DATA="../data/processed/trajectory_1000.csv"
	...
	try:
		...
		vehicle.setter('xgb', vehicle.detect_anomalies(b=15))
		...
		# Calculate speeds consistent with the new speeds
		...
		vehicle.setter('gdv', vehicle.smoother('dv', smoothing_window))
		...
		vehicle.setter('g_s', np.round(vehicle.smoother('v', smoothing_window), 4))

		# apply Gaussian filter on the new speeds
		vehicle.setter('g_ns', vehicle.smoother('new_speed', smoothing_window))

		# Calculate new accelerations from smoothed new speeds
		vehicle.setter('sg_na', vehicle.get_derivative('sg_ns')/3.6)
		vehicle.setter('g_na', np.round(vehicle.get_derivative('g_ns')/3.6, 4))

		vehicle.plot_process("../plots/")
		vehicle.df.rename(columns={"g_s": 'final_smoothed_speed', "g_na": "final_smoothed_acceleration"}, inplace=True)

		vehicle.save_data("../data/exp"+"_"+str(vehicle.df.id.unique())+".csv", 
						['id', 'frame', 'lon', 'lat', 'v', 'a_x', 'a_y', 'new_speed', 
						'final_smoothed_speed', 'final_smoothed_acceleration'])
